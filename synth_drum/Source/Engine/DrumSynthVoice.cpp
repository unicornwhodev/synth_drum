...
namespace
{
int resolvePadIndexForSettings(const PadSettings& settings)
{
    const int requested = juce::jlimit(0, kNumPads - 1, settings.padIndex);
    if (getPadVoiceModel(requested) == settings.voiceModel)
        return requested;

    for (int pad = 0; pad < kNumPads; ++pad)
        if (getPadVoiceModel(pad) == settings.voiceModel)
            return pad;
...
void DrumVoice::start(const PadSettings& settingsToUse, const float noteVelocity, const double currentSampleRate)
{
    ...
    bodyDampState = 0.0f;
    {
        ...
        bodyDamping = std::isfinite(config.damping) ? juce::jlimit(0.0f, 0.99f, config.damping) : 0.0f;
        if (bodyFeedback > 0.001f)
        {
            ...
            const float computedDelay = sr / bodyHz;
            bodyDelay = std::isfinite(computedDelay)
                ? juce::jlimit(1.0f, static_cast<float>(kBodyBufSize - 2), computedDelay)
                : 0.0f;
        }
        else
        {
...
void DrumVoice::render(juce::AudioBuffer<float>& buffer, const int startSample, const int numSamples)
{
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        advanceWrappedPhase(phase, pitchCurrentHz * pitchBendFactor);

        advanceWrappedPhase(subPhase, pitchCurrentHz * pitchBendFactor * 0.5f);

        advanceWrappedPhase(metalPhase, metalBaseHz);
        ...
        if (clickSamples > 0)
        {
            ...
            advanceWrappedPhase(clickPhase, clickBaseFreq + clickBaseFreq * 2.0f * clickProgress);
...
void DrumVoice::applyDedicatedStartShaping()
{
    switch (instrumentAlgorithm)
    {
        case DrumInstrumentAlgorithm::KickA:
            settings.baseFrequencyHz *= 0.94f;
            settings.pitchDropSemitones = juce::jlimit(0.0f, 48.0f, settings.pitchDropSemitones * 1.18f + 1.2f);
            settings.pitchDecaySeconds = juce::jlimit(0.004f, 1.2f, settings.pitchDecaySeconds * 1.12f);
            settings.decaySeconds = juce::jlimit(0.03f, 2.5f, settings.decaySeconds * 1.12f);
            settings.noiseAmount = clamp01f(settings.noiseAmount * 0.75f);
            settings.clickAmount = clamp01f(settings.clickAmount * 0.86f);
            break;

        case DrumInstrumentAlgorithm::KickB:
            settings.baseFrequencyHz *= 1.08f;
            settings.pitchDropSemitones = juce::jlimit(0.0f, 48.0f, settings.pitchDropSemitones * 0.72f + 0.7f);
            settings.pitchDecaySeconds = juce::jlimit(0.004f, 1.2f, settings.pitchDecaySeconds * 0.72f);
            settings.decaySeconds = juce::jlimit(0.03f, 2.5f, settings.decaySeconds * 0.78f);
            settings.clickAmount = clamp01f(settings.clickAmount * 1.25f + 0.05f);
            settings.cutoffHz = juce::jlimit(120.0f, 18000.0f, settings.cutoffHz * 1.12f);
            break;

        case DrumInstrumentAlgorithm::Snare:
            settings.noiseAmount = clamp01f(settings.noiseAmount * 1.18f + 0.05f);
            settings.clickAmount = clamp01f(settings.clickAmount * 1.22f + 0.035f);
            settings.decaySeconds = juce::jlimit(0.03f, 2.5f, settings.decaySeconds * 0.92f);
            settings.cutoffHz = juce::jlimit(120.0f, 18000.0f, settings.cutoffHz * 1.08f);
            break;

        case DrumInstrumentAlgorithm::Clap:
            settings.clapSpread = clamp01f(settings.clapSpread + 0.10f);
            settings.clapDensity = clamp01f(settings.clapDensity + 0.12f);
            settings.decaySeconds = juce::jlimit(0.03f, 2.5f, settings.decaySeconds * 1.08f);
            settings.pan = juce::jlimit(-1.0f, 1.0f, settings.pan + 0.04f);
            break;

        case DrumInstrumentAlgorithm::HatClosed:
            settings.openAmount = juce::jmin(settings.openAmount, 0.28f);
            settings.decaySeconds = juce::jlimit(0.006f, 2.5f, settings.decaySeconds * 0.58f);
            settings.metallicDensity = clamp01f(settings.metallicDensity + 0.14f);
            settings.clickAmount = clamp01f(settings.clickAmount + 0.02f);
            settings.cutoffHz = juce::jlimit(120.0f, 18000.0f, settings.cutoffHz * 1.12f);
            break;

        case DrumInstrumentAlgorithm::HatOpen:
            settings.openAmount = juce::jmax(settings.openAmount, 0.62f);
            settings.decaySeconds = juce::jlimit(0.03f, 2.5f, settings.decaySeconds * 0.92f);
            settings.metallicDensity = clamp01f(settings.metallicDensity + 0.08f);
            settings.noiseAmount = clamp01f(settings.noiseAmount + 0.04f);
            settings.cutoffHz = juce::jlimit(120.0f, 18000.0f, settings.cutoffHz * 0.96f);
            break;

        case DrumInstrumentAlgorithm::Perc1Wood:
            settings.bodyTone = clamp01f(settings.bodyTone * 0.72f);
            settings.modalRing = clamp01f(settings.modalRing + 0.12f);
            settings.pitchDropSemitones = juce::jlimit(0.0f, 48.0f, settings.pitchDropSemitones * 1.10f);
            settings.cutoffHz = juce::jlimit(120.0f, 18000.0f, settings.cutoffHz * 0.88f);
            break;

        case DrumInstrumentAlgorithm::Perc2Metal:
            settings.bodyTone = juce::jmax(settings.bodyTone, 0.62f);
            settings.modalRing = clamp01f(settings.modalRing + 0.22f);
            settings.pitchDropSemitones = juce::jlimit(0.0f, 48.0f, settings.pitchDropSemitones * 0.88f);
            settings.cutoffHz = juce::jlimit(120.0f, 18000.0f, settings.cutoffHz * 1.18f);
            settings.clickAmount = clamp01f(settings.clickAmount + 0.025f);
            break;

        case DrumInstrumentAlgorithm::TomLow:
            settings.baseFrequencyHz *= 0.88f;
            settings.decaySeconds = juce::jlimit(0.03f, 2.5f, settings.decaySeconds * 1.18f);
            settings.pitchDropSemitones = juce::jlimit(0.0f, 48.0f, settings.pitchDropSemitones * 1.16f + 0.4f);
            settings.bodyTone = clamp01f(settings.bodyTone + 0.14f);
            settings.clickAmount = clamp01f(settings.clickAmount * 0.82f);
            break;

        case DrumInstrumentAlgorithm::TomHigh:
            settings.baseFrequencyHz *= 1.10f;
            settings.decaySeconds = juce::jlimit(0.03f, 2.5f, settings.decaySeconds * 0.82f);
            settings.pitchDropSemitones = juce::jlimit(0.0f, 48.0f, settings.pitchDropSemitones * 0.80f);
            settings.clickAmount = clamp01f(settings.clickAmount + 0.035f);
            settings.cutoffHz = juce::jlimit(120.0f, 18000.0f, settings.cutoffHz * 1.10f);
            break;

        case DrumInstrumentAlgorithm::Crash:
            settings.openAmount = juce::jmax(settings.openAmount, 0.78f);
            settings.decaySeconds = juce::jlimit(0.05f, 2.5f, settings.decaySeconds * 1.35f);
            settings.metallicDensity = clamp01f(settings.metallicDensity + 0.16f);
            settings.noiseAmount = clamp01f(settings.noiseAmount + 0.07f);
            settings.clickAmount = clamp01f(settings.clickAmount * 0.62f);
            break;

        case DrumInstrumentAlgorithm::Fx:
            settings.fmIndex = clamp01f(settings.fmIndex + 0.18f);
            settings.fmSweep = clamp01f(settings.fmSweep + 0.12f);
            settings.pitchDropSemitones = juce::jlimit(0.0f, 48.0f, settings.pitchDropSemitones * 1.10f);
            settings.clickAmount = clamp01f(settings.clickAmount + 0.04f);
            settings.noiseAmount = clamp01f(settings.noiseAmount + 0.02f);
            break;
    }
}
...
float DrumVoice::safePhaseIncForFrequency(const float frequencyHz) const noexcept
{
    const float sr = static_cast<float>(std::max(1.0, sampleRate));
    if (!std::isfinite(frequencyHz) || frequencyHz <= 0.0f)
        return 0.0f;
    return juce::jlimit(0.0f, 0.45f, frequencyHz / sr);
}

float DrumVoice::harmonicNyquistScale(const float ratio) const noexcept
{
    const float absRatio = std::abs(ratio);
    if (!std::isfinite(absRatio) || absRatio <= 0.0f)
        return 0.0f;

    const float inc = safePhaseIncForFrequency(pitchCurrentHz * pitchBendFactor) * absRatio;
    if (!std::isfinite(inc))
        return 0.0f;
    if (inc <= 0.42f)
        return 1.0f;
    if (inc >= 0.45f)
        return 0.0f;
    return (0.45f - inc) / 0.03f;
}

void DrumVoice::advanceWrappedPhase(float& targetPhase, const float frequencyHz) const noexcept
{
    targetPhase += safePhaseIncForFrequency(frequencyHz);
    if (!std::isfinite(targetPhase))
    {
        targetPhase = 0.0f;
        return;
    }
...
{
    if (!std::isfinite(delaySamples) || delaySamples < 1.0f
        || delaySamples > static_cast<float>(kBodyBufSize - 2))
        return 0.0f;
...
void TonalDrumVoice::renderModel(float& body, float& noise, float& click)
{
    ...
    {
        float h2 = mds::fastSin(phase * 2.0f) * cfg.harmonic2 * harmonicNyquistScale(2.0f);
        if (cfg.pitchContourOnH2) h2 *= pitchContour;
        signal += h2;
    }

    if (cfg.harmonic3 > 0.0f)
        signal += mds::fastSin(phase * 3.0f) * cfg.harmonic3 * harmonicNyquistScale(3.0f);

    if (cfg.goldenRatioLevel > 0.0f)
        signal += mds::fastSin(phase * k::kGoldenRatio) * cfg.goldenRatioLevel * harmonicNyquistScale(k::kGoldenRatio);
    ...
    if (usesDedicatedV2())
    {
        ...
        {
            case DrumInstrumentAlgorithm::KickA:
                signal += mds::fastSin(subPhase * 0.55f) * 0.075f;
            ...
            case DrumInstrumentAlgorithm::TomLow:
                signal += mds::fastSin(subPhase) * 0.055f;
                signal += mds::fastSin(phase * 1.28f) * 0.030f * harmonicNyquistScale(1.28f);
                noise *= 0.82f;
                click *= 0.88f;
                break;
...
void MetallicDrumVoice::renderModel(float& body, float& noise, float& click)
{
    ...
    for (int p = 0; p < activeMetallicPartials; ++p)
    {
        ...
        const float ratio = partials.ratios[p];
        const float inc = safePhaseIncForFrequency(metalBaseHz) * ratio;
        const float scale = inc <= 0.42f ? 1.0f : (inc >= 0.45f ? 0.0f : (0.45f - inc) / 0.03f);
        if (scale > 0.0f)
            metal += mds::fastSin(metalPhase * ratio) * partials.amplitudes[p] * densityWeight * scale;
    }
...
void ModalDrumVoice::renderModel(float& body, float& noise, float& click)
{
    ...
    float signal = mds::fastSin(phase) * (cfg.sineLevel * (0.78f + bodyTone * 0.48f))
                 + mds::fastSin(phase * 2.0f) * (cfg.harmonic2 * (1.18f - bodyTone * 0.36f) * harmonicNyquistScale(2.0f));

    if (cfg.hasMetalPartials)
    {
        const float metallicBias = 0.40f + bodyTone * 0.90f;
        signal += mds::fastSin(phase * k::kGoldenRatio) * k::kModalGoldenPartialAmp * metallicBias * harmonicNyquistScale(k::kGoldenRatio)
                + mds::fastSin(phase * k::kSqrt6Approx) * k::kModalSqrt6PartialAmp * metallicBias * harmonicNyquistScale(k::kSqrt6Approx);
    }
    ...
    if (usesDedicatedV2())
    {
        switch (instrumentAlgorithm)
        {
            ...
            case DrumInstrumentAlgorithm::Perc2Metal:
                body += mds::fastSin(phase * 2.82f) * tailEnv * 0.035f * harmonicNyquistScale(2.82f);
...
void FxVoice::renderModel(float& body, float& noise, float& click)
{
    ...
    float modDepth = k::kFmBaseModDepth * (0.35f + fmIndex * 1.85f)
                         + noiseEnvelope * k::kFmModEnvScale * (0.30f + fmSweep * 1.40f);
    const float carrierInc = safePhaseIncForFrequency(pitchCurrentHz * pitchBendFactor);
    const float sidebandBudget = 0.45f - carrierInc * (1.0f + modFreq);
    if (!std::isfinite(sidebandBudget) || sidebandBudget <= 0.0f)
        modDepth = 0.0f;
    else
        modDepth *= juce::jlimit(0.0f, 1.0f, sidebandBudget / 0.08f);
    ...
    if (usesDedicatedV2())
    {
        const float sideRatio = 1.75f + fmSweep * 1.30f;
        const float sideScale = harmonicNyquistScale(sideRatio);
        const auto sideband = mds::fastSin(phase * sideRatio + mds::fastSin(phase * (0.42f + fmIndex)) * modDepth * 0.45f);
