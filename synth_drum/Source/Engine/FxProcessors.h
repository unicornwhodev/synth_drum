...
public:
    ...
    void process(float* left, float* right, int numSamples, const Params& p)
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            left[i] = dryL * dry + dcOutL * mix;
            if (right != nullptr)
                right[i] = dryR * dry + dcOutR * mix;
        }
    }
...
public:
    ...
    void process(float* left, float* right, int numSamples, const Params& p)
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            // 4x oversample for true peak detection (linear interpolation upsample)
